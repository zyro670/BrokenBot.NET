    ...
    public class TwitchBot<T> where T : PKM, new()
    {
        ...
        private string HandleCommand(TwitchLibMessage m, string c, string args, bool whisper)
        {
            ...
            switch (c)
            {
                ...
                case "pr":
                    return Info.Hub.Ledy.Pool.Reload(Hub.Config.Folder.GiveawayFolder) ? $"Reloaded from folder. Pool count: {Info.Hub.Ledy.Pool.Count}" : "Failed to reload from folder.";
