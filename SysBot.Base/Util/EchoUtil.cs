...
public static class EchoUtil
{
    ...
    public static readonly List<Action<string, Embed>> EmbedForwarders = [];
    public static readonly List<Action<byte[], string, EmbedBuilder>> RaidForwarders = [];
    ...
    public static void EchoEmbed(string ping, string message, string url, string markurl, bool result)
    {
        foreach (var fwd in EmbedForwarders)
        {
            try
            {
                ...
                var embed = new EmbedBuilder { Color = result ? Color.Teal : Color.Red, ThumbnailUrl = url }.WithAuthor(author).WithDescription(message);
