...
namespace SysBot.Pokemon.Discord
{
    ...
    public class PKMSVShowdownSetCommands<T> : ModuleBase<SocketCommandContext> where T : PKM, new()
    {
        public static Dictionary<string, PKMSVShowdownSets> SVShowdownSetsCollection = PKMSVModule.SVShowdownSetsCollection;
        ...
        public async Task CallForShowdownSets(params string[] userSetParts)
        {
            ...
            List<PKMSVShowdownSets> matchedSets = new List<PKMSVShowdownSets>();

            if (userSetParts.Length == 1)
            {
                foreach (var set in SVShowdownSetsCollection)
                {
            ...
            {
                if (SVShowdownSetsCollection.TryGetValue(userSet, out var specificSet))
                {
            ...
            if (matchedSets.Count > 0)
            {
                foreach (var goodSet in matchedSets)
                {
                    ...
                    if (goodSet.TeraType != null) message.AppendLine($"Tera Type: {goodSet.TeraType}");
