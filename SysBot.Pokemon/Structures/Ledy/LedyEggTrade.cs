...
{
    public class LedyEggTrade<T> where T : PKM, new()
    {
        public readonly Dictionary<string, LedyRequest<T>> UserRequests = new();
        public readonly Dictionary<string, LedyRequest<T>> EggTrade;

        public readonly PokemonPool<T> EggTradePool;
        private readonly List<LedyUser> Previous = new();

        public LedyEggTrade(PokemonPool<T> eggtradepool)
        {
            EggTradePool = eggtradepool;
            EggTrade = EggTradePool.Files;
        }
        ...
        private LedyResponse<T>? GetLedyResponse(T pk)
        {
            ...
            if (EggTrade.TryGetValue(nick, out match))
