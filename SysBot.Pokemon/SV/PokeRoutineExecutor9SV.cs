...
public abstract class PokeRoutineExecutor9SV : PokeRoutineExecutor<PK9>
{
    ...
    public async Task<byte> ReadEncryptedBlockByte(ulong baseBlock, DataBlock block, CancellationToken token)
    {
        var addr = await SearchSaveKey(baseBlock, block.Key, token).ConfigureAwait(false);
