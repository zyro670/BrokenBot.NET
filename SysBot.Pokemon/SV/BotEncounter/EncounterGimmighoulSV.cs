...
namespace SysBot.Pokemon.SV.BotEncounter
{
    public class EncounterGimmighoulSV : EncounterBotSV
    {
        private int _encounterCount;

        public EncounterGimmighoulSV(PokeBotState cfg, PokeTradeHub<PK9> hub)
            : base(cfg, hub)
        {
        }
            ...
            while (!token.IsCancellationRequested)
            {
                ...
                while (b1s1 == null || (Species)b1s1.Species == Species.None)
                {
                    ...
                    if (b1s1 != null && b1s1.EncryptionConstant != null && (Species)b1s1.Species != Species.None)
                    {
                        ...
                        _settings.AddCompletedGimmighouls();
                        TradeExtensions<PK9>.EncounterLogs(b1s1, "EncounterLogPretty_GimmighoulSV.txt");
                        TradeExtensions<PK9>.EncounterScaleLogs(b1s1, "EncounterLogScalePretty.txt");

                        if (_dumpSetting.Dump)
                            DumpPokemon(_dumpSetting.DumpFolder, "gimmighoul", b1s1, true);

                        if (_dumpSetting.DumpRaw && bytes != null)
                            DumpPokemon(_dumpSetting.DumpFolder, "gimmighoul", b1s1, bytes);
